...
# def perform_sql(text):
#     sql_model = ModelCatalog().load_model("slim-sql-tool")
#     response_sql = sql_model.function_call(text)
#     return response_sql

def perform_sql(text):
    sql_model = ModelCatalog().load_model("slim-ner-tool")
    response_sql = sql_model.function_call(text)
    return response_sql
